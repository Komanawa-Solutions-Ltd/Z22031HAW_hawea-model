...
from komanawa.hawea.hawea_base import campbells_dir, proj_root
from komanawa.hawea.Scenarios.wetland_setback_campbells.project_model_tools import smt, tdis
from komanawa.hawea.Scenarios.scen_period import scen_tdis
from komanawa.hawea.Scenarios.supporting_data_analysis.pumping_data import get_pump_curve
from komanawa.hawea.Scenarios.boundary_conditions import get_scen_rch, _get_str_stage_flow
...
def get_strt_hds(recalc=False):
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        uncompress_model(proj_root.joinpath('optimisation/final_opt_models/3d_v1d'),
...
def get_rch(recalc=False):
    save_path = campbells_dir.joinpath('processed_input_data', 'rch_data.npz')
    if save_path.exists() and not recalc:
...
def get_riv(conductance, recalc=False):
    save_path = campbells_dir.joinpath('processed_input_data', 'riv_data.npz')
    if save_path.exists() and not recalc:
