...
from komanawa.hawea.Scenarios.wetland_setback_campbells.params import get_hk, get_sy
from komanawa.hawea.model_build.modflow_model import build_model
from komanawa.hawea.Scenarios.wetland_setback_campbells.project_model_tools import tdis, smt
from komanawa.hawea.Scenarios.wetland_setback_campbells.model_bcs import get_riv, get_wells, get_rch, get_wetland_loc, get_strt_hds
...
def run_model_extrac_data(model_name, model_ws, locs, max_pumping_rate, hk_modifer,
                          sy_modifer, riv_cond, rm_files=True, keep_list=False):
    ...
    with model_ws.joinpath(f'{model_name}{inputs_suffix}').open('w') as f:
        ...
        f.write(f'{hk_modifer = }\n')
        f.write(f'{sy_modifer = }\n')
        f.write(f'{riv_cond = }\n')
        if well_loc is not None:
            for k, i, j in well_loc.itertuples(False, None):
                f.write(f'well_loc = {k},{i},{j}\n')
    hk = get_hk(hk_modifer)
    sy = get_sy(sy_modifer)
...
def get_ssh_dist(local_cores, external_ips):
    ...
    ssh_dist = SshDist(
        ...
        script_path=opt_proj_root.joinpath('Scenarios/wetland_setback_campbells/run_scenario.py'),
        conda_env='hawea',
        num_cores=num_cores,
