...
from komanawa.hawea.hawea_base import proj_root, processed_scen_dir
...
def get_old_allo_zones(recalc=False):
    shp_path = proj_root.joinpath('Scenarios/base_data/scott_zones/scott_zones.shp')
    save_path = processed_scen_dir.joinpath('old_allo_zones.npy')
    data = gpd.read_file(shp_path)
    mapper = data.set_index('ID')['Zone'].to_dict()
    if save_path.exists() and not recalc:
        return np.load(save_path), mapper
...
def plot_old_allocation_zone(save=False):
    plot_data, mapper = get_old_allo_zones()
    mapper_keys = list(mapper.keys())
    plot_data[~np.isfinite(plot_data)] = -1
    colors = {i: c for i, c in zip(mapper_keys, smt.plot.get_colors(mapper_keys, 'tab20'))}
    use_colors = [
    ...
    if not save:
        ...
        fig.savefig(outdir.joinpath('old_allo_zones.png'))


if __name__ == '__main__':
    plot_old_allocation_zone(True)
    get_allo_zones(True)
