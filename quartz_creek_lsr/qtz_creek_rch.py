...
layer_type = [0]
model_version_name = 'qtz_creek'
base_map_path = proj_root.joinpath('model_build/base_data/nz-topo50-maps.jpg')
default_figsize = (8, 8)


def get_ibound():
    return np.isfinite(smt.io.shape_file_to_model_array(boundary_path, 'id', alltouched=True))[np.newaxis, :, :].astype(
        int)
...
def get_soil_classes(show=False):
    ...
    if show:
        smt.plot.show()
    soil_classes = soil_classes.astype(int)
    return soil_classes


processed_scen_dir = proj_root.joinpath('quartz_creek_lsr/results')
...
def _make_corrections():
    ...
    ibound = smt_haw.get_no_flow(0)
    irrigated = {}
    for y in [2015, 2020, 2021]:
        t = get_irrigation_code_haw(y)
...
def _predict_scenario_data(ibound, regr_irrig, regr_unirrig, dryland=False):
    ...
    if dryland:
        ...
        t = get_irrigation_code(y, )  # code is only to identify irrigated area
...
def sumerize_plot_rch(): #
    t = get_irrigation_code(2021)
    smt.plot.plt_matrix(t, base_map=True, no_flow_layer=0)
    dates, irr = get_corrected_scenario_era5_rch(start_date=None, end_date=None, dryland=False, frequency='Y',
                                                 fun='mean', )

    dates, dry = get_corrected_scenario_era5_rch(start_date=None, end_date=None, dryland=True, frequency='Y',
                                                 fun='mean', )
    outdata = pd.DataFrame(index=dates, columns=['irrigated_mm_y', 'dryland_mm_y'])
    outdata['irrigated_mm_y'] = np.nanmean(irr, axis=(1, 2)) * 365
    outdata['dryland_mm_y'] =  np.nanmean(dry, axis=(1, 2)) * 365
    outdata.to_csv(processed_scen_dir.joinpath('rch_summary.csv'))

    mean_irr = irr.mean(axis=0)*365
    mean_dry = dry.mean(axis=0)*365
    modelled = smt.get_no_flow(0)==1
    smt.io.array_to_raster(processed_scen_dir.joinpath('rch_irr_mean_mm_y.tif'), mean_irr, 0)
    smt.io.array_to_raster(processed_scen_dir.joinpath('rch_dry_mean_mm_y.tif'), mean_dry, 0)
    vmax = np.nanpercentile(np.concatenate([mean_irr[modelled], mean_dry[modelled]]), 99)
    vmin = np.nanpercentile(np.concatenate([mean_irr[modelled], mean_dry[modelled]]), 1)
    print(pd.Series(mean_irr[modelled]).describe())

    fig, ax = smt.plot.plt_matrix(mean_irr, vmin=vmin, vmax=vmax, title='Mean annual irrigated recharge (mm/yr)', base_map=True,
                                    no_flow_layer=0)
    fig.tight_layout()
    fig.savefig(processed_scen_dir.joinpath('rch_irr_mean_mm_y.png'))

    fig, ax = smt.plot.plt_matrix(mean_dry, vmin=vmin, vmax=vmax, title='Mean annual dryland recharge (mm/yr)', base_map=True,
                                    no_flow_layer=0)
    fig.tight_layout()
    fig.savefig(processed_scen_dir.joinpath('rch_dry_mean_mm_y.png'))

    fig, ax = plt.subplots()
    ax.violinplot([outdata['irrigated_mm_y'], outdata['dryland_mm_y']])
    ax.set_ylabel('Mean annual recharge (mm/yr)')
    ax.set_title('Mean annual recharge (mm/yr)')
    labels = ['irrigated', 'dryland']
    ax.set_xticks([1, 2])
    ax.set_xticklabels(labels)
    fig.tight_layout()
    fig.savefig(processed_scen_dir.joinpath('rch_violin.png'))
