"""
created matt_dumont 
on: 15/03/23
"""
import numpy as np

from Scenarios.wetland_setback_butterfield.project_model_tools import smt
from project_base import butterfield_dir, base_model_build_data_dir
from model_parameterisation.optimised_parameterisation import get_3d_v1d_params
from model_parameterisation.pilot_points import interpolate_kh_pilot_points, interpolate_sy_pilot_points


def get_terrace_zone(recalc=False):
    shp_path = base_model_build_data_dir.joinpath('zones/terrace.shp')
    save_path = butterfield_dir.joinpath('processed_input_data/terrace_zone.npy')

    if recalc:
        return np.load(save_path)
    out = smt.io.shape_file_to_model_array(shp_path, 'id', alltouched=True)
    out = np.isfinite(out)
    np.save(save_path, out)
    return out


def get_hk(terrace_hk, flat_hk, recalc=False):

    kh_param, sy_param, riv_params, hill_param, race_param, rch_param = get_3d_v1d_params()
    hk = interpolate_kh_pilot_points(kh_param)[0]

    # todo pull from hawea model (then add multipler)

    out = smt.get_model_zeros(True) + flat_hk
    out[:, get_terrace_zone()] = terrace_hk
    return out


def get_sy(terrace_sy, flat_sy):
    # todo pull from hawea model (then add multipler)
    kh_param, sy_param, riv_params, hill_param, race_param, rch_param = get_3d_v1d_params()


    out = smt.get_model_zeros(True) + flat_sy
    out[:, get_terrace_zone()] = terrace_sy
    return out
